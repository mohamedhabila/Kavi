...
/** Strict schema codec and invariant validator for one native durable execution record. */
internal object AndroidDurableExecutionRecordCodec {
  ...
  fun encode(record: AndroidDurableExecutionRecord): String = encodeRecord(record).toString()

  fun decode(serialized: String): AndroidDurableExecutionRecord? =
    decodeRecord(JSONObject(serialized))

  fun isValid(record: AndroidDurableExecutionRecord): Boolean {
    ...
    if (
      ...
      !isValidPlatformWorkId(record.platformWorkId) ||
      record.revision < 0 ||
  ...
  fun isValidPlatformWorkId(value: String): Boolean = try {
