...
describe('recallFactsForQuery - workflow support ranking', () => {
  ...
  it('ranks compact popup option facts above noisy page inventories', async () => {
    ...
    const scored = await recallScoredFactsForQuery('qforum-control qsort-current qoption-beta', {
      conversationId,
      memoryKind: ['ui_inventory', 'ui_field'],
      limit: 1,
      threshold: 0.01,
