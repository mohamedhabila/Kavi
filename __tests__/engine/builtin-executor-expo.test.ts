// ---------------------------------------------------------------------------
// Builtin Tool Executor — Expo-specific tests
// ---------------------------------------------------------------------------
...
import * as expoProjectCreation from '../../src/services/expo/projectCreation';
import * as expoProjectSync from '../../src/services/expo/projectSync';
...
describe('Builtin Tool Executor Expo wrappers', () => {
  beforeEach(() => {
    ...
    (expoProjectAutomation.getExpoProjectReadinessLabel as jest.Mock).mockReturnValue('Ready');
  ...
  it('returns lean project listings without repeated automation metadata', async () => {
    (expoProjectSync.listExpoProjects as jest.Mock).mockResolvedValue([
      {
        id: 'expo-project-1',
        easProjectId: 'eas-project-1',
        name: 'Kavi',
  ...
  it('redirects create-project calls to an existing resolved project unless creation is confirmed', async () => {
    ...
    (expoProjectResolution.resolveExpoProjectForExecutionTask as jest.Mock).mockResolvedValue({
      status: 'resolved',
  ...
  it('returns concise summaries for probe and manual action responses', async () => {
    (expoWorkflowActions.probeExpoProject as jest.Mock).mockResolvedValue({
      ok: true,
      message: 'Expo workflow ready',
      checkedAt: 123,
      checks: [],
    });
    (expoWorkflowActions.runExpoProjectAction as jest.Mock).mockResolvedValue({
      mode: 'eas-workflow',
      workflowRun: {
        id: 'run-1',
  ...
  it('keeps workflow status payloads failure-focused and trimmed', async () => {
    (expoWorkflowMonitoring.inspectExpoWorkflowRun as jest.Mock).mockResolvedValue({
      status: 'ok',
