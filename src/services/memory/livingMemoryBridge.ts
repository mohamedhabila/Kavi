...
export const CURRENT_MEMORY_PRESENTATION_CONTRACT =
  'Memory presentation contract: Retrieved Memory contains facts that product code has already checked for current scope, validity, authority, conflicts, sensitivity, and direct usability. Treat facts labeled policy=use as available context for the requested answer or action; their predicate may be a canonical identifier, so interpret subject, predicate, and value together. The factId field is the opaque identity accepted by memory management tools; source is provenance and must not be used as factId. Do not ask the user to repeat them or call a memory read tool merely to verify a fact already shown here. Re-read memory only when the needed fact is absent, broader history is requested, or current evidence conflicts. Do not disclose superseded or historical values merely to explain a current answer; disclose them only when the user actually asks for history or comparison.';
