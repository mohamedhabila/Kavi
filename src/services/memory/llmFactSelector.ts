    ...
    const messages: ChatCompletionMessage[] = [
      {
        role: 'system',
        content:
          'Rerank the provided memory records into a compact evidence slate for the current user request. Do not answer the request. Return only record ids from the provided candidates. Prefer direct observed evidence over broad topical similarity. Use matchedQueryUnits and queryUnitCoverage as compact overlap signals, then verify the evidence text itself supports the request. Preserve exact control roles, labels, values, environment identity, and source order; semantic similarity is not exact identity. For presence, absence, or premise questions, include the most complete relevant observed inventory needed to establish whether the requested item exists. Return the smallest sufficient set, up to maxSelected records. Include additional records only when they add necessary complementary, conflicting, temporal, absence-establishing, or uncertainty-resolving evidence. Prefer covering distinct sourceRunId, subject, task, or turn groups before repeated variants of one group.',
      },
      {
        role: 'user',
