...
// Kavi — Temporal Decay
// ---------------------------------------------------------------------------
// Adapted: node:fs → expo-file-system, node:path → inline
...
async function extractTimestamp(params: {
  ...
  const absolutePath = params.filePath.startsWith('file://')
    ? params.filePath
    : `${params.workspaceDir.replace(/\/$/, '')}/${params.filePath}`;
...
>(params: {
  ...
  const timestampCache = new Map<string, Promise<Date | null>>();

  return Promise.all(
    params.results.map(async (entry) => {
      const cacheKey = `${entry.source}:${entry.path}`;
      let timestampPromise = timestampCache.get(cacheKey);
      if (!timestampPromise) {
        ...
        timestampCache.set(cacheKey, timestampPromise);
      }
