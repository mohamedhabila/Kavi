...
function prepareJournal(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (row) {
      const snapshot = readSnapshot(database, plan.identity);
      assertPlanMatchesExisting(database, plan, snapshot);
      return snapshot;
    }
    insertPlannedJournal(database, plan);
    return readSnapshot(database, plan.identity);
  });
}
...
function cancelUnclaimedDispatch(
  ...
  appendTerminalCheckpoint(database, run, effect.id, effect.requestDigest, cancelledAt);
...
function updateRunAfterSettlement(
  ...
  if (terminalAt !== null) {
    appendTerminalCheckpoint(database, run, effectId, outcomeDigest, observedAt);
  }
  const update = database.runSync(
    `UPDATE execution_runs
...
function makePorts(input: {
  ...
  const readState = async (): Promise<EffectDispatchReadState | null> => {
    ...
    const snapshot = readSnapshot(database, input.identity);
...
export function prepareToolEffectDispatchJournal(
  ...
  const snapshot = prepareJournal(database, plan);
  assertPlanMatchesExisting(database, plan, snapshot);
  return {
