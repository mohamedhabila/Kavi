...
export async function executeWait(args: {
  ms?: number;
  reason?: string;
...
export async function executePdfRead(args: {
  path: string;
  pages?: string;
}): Promise<ToolRuntimeOutcome> {
  if (args.path.startsWith('http://') || args.path.startsWith('https://')) {
    try {
      ...
      if (!['http:', 'https:'].includes(url.protocol)) {
        return failedToolOutcome(JSON.stringify({ error: 'Only http/https URLs are supported' }));
      }
      ...
      return failedToolOutcome(
        JSON.stringify({ error: `HTTP ${htmlRes.status} fetching PDF URL` }),
      );
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      return failedToolOutcome(JSON.stringify({ error: message }));
    }
  }
