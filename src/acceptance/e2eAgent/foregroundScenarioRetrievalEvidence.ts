...
function projectClosedEvent(event: MemoryRetrievalEvent): MemoryRetrievalEvent {
  return {
    ...
    candidates: {
      strategy: event.candidates.strategy,
      localSimilarityOutcome: event.candidates.localSimilarityOutcome,
      eligibleScanCount: event.candidates.eligibleScanCount,
      pinnedCount: event.candidates.pinnedCount,
      exactQuotedCount: event.candidates.exactQuotedCount,
      lexicalCount: event.candidates.lexicalCount,
      entityCount: event.candidates.entityCount,
      temporalCount: event.candidates.temporalCount,
      localSimilarityCount: event.candidates.localSimilarityCount,
      unionCount: event.candidates.unionCount,
      diversifiedCount: event.candidates.diversifiedCount,
      unionMs: event.candidates.unionMs,
    },
    expansion: {
