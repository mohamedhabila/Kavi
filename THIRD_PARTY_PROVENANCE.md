...
- generated assets built from third-party editor and terminal packages
...
| `patches/image-size+1.2.1.patch`                                                                                                                       | `image-size` (via `metro`)                          | Verified                        | MIT     | Local patch bounds the ICNS, JXL, and HEIF container walks that hang on a zero-size box (GHSA-w3rx-r6r6-pgpr, GHSA-5p2g-fcmc-qvqq). Build-time only: the parser runs in Metro's asset pipeline, not in the shipped app. |
...
| `assets/terminal/xterm.html`, `android/app/src/main/assets/terminal/xterm.html`, `assets/terminal/runtime/*`, `scripts/build-terminal-assets.js`       | xterm packages (`@xterm/*`)                         | Verified                        | MIT     | Terminal HTML is generated locally from the pinned xterm packages during `npm install` and `npm run build:terminal-assets`. The generated renderer is committed so the terminal does not fetch executable CDN code.      |
...
The `image-size` patch is retained because upstream has published no fixed release: both advisories cover every published version, so there is no version to upgrade to. `image-size` reaches the tree through `metro` -> `@expo/metro` -> `expo`, and it selects a parser by sniffing magic bytes rather than by file extension, so an image asset named `.png` whose bytes are an ICNS or JXL container reaches the affected parsers and can wedge a bundle build or dev server until the process is killed. `__tests__/scripts/imageSizeParserHardening.test.ts` fails if the patch stops being applied. Drop the patch once a fixed `image-size` release is available and `metro` depends on it.

If either patch is ever upstreamed or replaced, update this file and `CONTRIBUTING.md` at the same time.
