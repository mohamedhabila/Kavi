...
| `patches/image-size+1.2.1.patch`                                                                                                                       | `image-size` (via `metro`)                          | Verified                        | MIT     | Local patch bounds the ICNS and JXL container walks that hang on a zero-size box (GHSA-w3rx-r6r6-pgpr, GHSA-5p2g-fcmc-qvqq). Build-time only: the parser runs in Metro's asset pipeline, not in the shipped app.        |
