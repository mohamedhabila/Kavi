...
the 2026-07-28 source, dependency, and evaluation refresh:

| Check                                    | Result        | Notes                                                                                                                                                                                                                                                                    |
| ---------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `npm run verify`                         | Pass          | Public hygiene/language, links, 1,033 dependency-license entries, evaluation contracts, metadata, dependency alignment, nine locales, architecture, maintainability, lint, TypeScript, and 1,200 suites / 10,564 tests passed; 16 opt-in suites / 81 tests were skipped. |
| `npm run test:coverage`                  | Pass          | Existing thresholds passed without reduction: 86.48% statements, 76.99% branches, 91.52% functions, and 87.70% lines.                                                                                                                                                    |
...
| `npm run eval:long-tasks`                | Pass          | The keyless structural durability gate passed 12 suites / 100 tests across interrupted foreground replies, scheduler persistence, restart recovery, native recovery bridges, terminal reporting, and duplicate-effect prevention. Physical-device lifecycle evidence is still required. |
...
| Production and full high-severity audits | Needs signoff | No critical advisories. The current audit reports 18 production / 34 total high findings, all expanded from the newly published `brace-expansion` advisory in legacy Expo, React Native, and Jest build tooling, plus 8 moderate findings from Expo's build-time `xcode -> uuid` chain. The compatible MCP SDK update removed the former Hono finding. npm's forced high-severity remediation requires an unsupported React Native or Expo major change; see `docs/release.md` for the reachability analysis and release-blocking conditions. |
