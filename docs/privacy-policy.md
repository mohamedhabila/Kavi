...
This Privacy Policy explains what information Kavi can handle, how it is used,
when information may leave your device, and what controls are available to you.

## Information Kavi Can Handle

Kavi can handle the following categories of information when you choose to use
the related feature:

1. Prompts, messages, conversation history, and other content you enter into
   the app.
2. Photos, videos, documents, and other files you choose to attach, import,
   share, or capture for a conversation or tool action.
3. Voice notes, microphone input, and audio captured during a user-requested
   recording.
4. Contacts and calendar information used for user-requested productivity actions.
5. Location information used for user-requested maps or location-aware actions.
6. Configuration data for user-added integrations, such as AI provider
   settings, MCP servers, SSH targets, remote workspace URLs, browser-worker
   endpoints, and similar connection details.
7. Sensitive credentials you choose to save in the app, such as provider API
   keys, OAuth secrets, SSH credentials, and workspace tokens.
8. Local app state such as settings, enabled permissions, selected providers,
   and locally stored conversation data.

## How Kavi Uses Information

Kavi uses information only to provide the features you request, including to:
...
Kavi supports both on-device and remote processing.

1. If you select a supported on-device model, prompts and inference can stay on
   your device.
2. If you configure a remote AI provider, MCP server, SSH target, remote
   workspace, browser worker, or similar service, Kavi sends the minimum data
   needed for the user-requested action to that configured service.
3. Kavi does not silently enable or auto-discover remote services. Remote
   access depends on settings and connections that you explicitly provide.
4. Kavi's own core app runtime does not require a Kavi-operated gateway.

## Third-Party Services And Sharing

Kavi shares information only when a feature requires it or when you configure a
third-party service and ask Kavi to use it. Depending on your configuration,
this can include AI model providers, remote hosts, remote workspaces, browser
automation workers, MCP servers, and other integrations you explicitly add.
...
Kavi requests sensitive permissions only when the related feature is used.
...
1. In release builds, sensitive secrets such as API keys, OAuth secrets, SSH
   credentials, and workspace tokens are intended to be stored using platform
   secure storage.
2. General app settings, local conversation data, and other non-secret state may
   be stored locally on your device so the app can restore your experience
   between launches.
...
5. If secure credential deletion fails, Kavi keeps the provider or integration visible so you can retry the deletion.
6. Data sent to a third-party service that you configure is subject to that service's retention practices and policies.
...
If you configure a third-party service hosted outside your country or region,
information sent to that service may be processed in other jurisdictions. The
handling of that information is governed by the terms and privacy practices of
the service you choose.

## Children

Kavi is not directed to children.

## Changes To This Policy

This Privacy Policy may be updated from time to time. If it changes, the
updated version will be published at the same public URL with a revised date.

## Contact

For privacy or support questions, use the support contact listed on the public app listing or support website for Kavi.
