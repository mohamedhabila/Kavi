...
| Local long-term memory                                                                  | Yes | Yes     | Available         | Stored on-device in SQLite; Settings can disable it. Explicit source preservation stores bounded current-user text owner-wide. Consolidation or embedding may call a configured provider.                           | Memory policy, retrieval, ingestion, source withdrawal, opt-out, and acceptance metric tests.              |
...
| Local JavaScript terminal                                                               | Yes | Yes     | Available         | The xterm renderer and addons are bundled into the app; the local REPL does not need a network connection. JavaScript execution remains a trusted-user utility, not a security sandbox.                             | Offline Release flow, WebView asset parity, terminal bridge, and JavaScript utility tests.                 |
| SSH, SFTP, and SSH terminal sessions                                                    | Yes | Yes     | Setup required    | The terminal renderer is bundled. Remote sessions require a reachable host, accepted host key policy, and user-supplied credentials or key material.                                                                | SSH connector, session store, terminal, SFTP, and platform safety tests.                                   |
...
| Camera, media attachments, and media understanding                                      | Yes | Yes     | Permission-gated  | Camera capture requires camera permission. Image selection uses the operating-system picker and exposes only user-selected media. Provider-side media support depends on the active model.                          | Media attachment, picker, camera, image generation, image editing, and provider capability tests.          |
...
| Scheduled workflows and reminders                                                       | Yes | Yes     | Permission-gated  | Two distinct primitives. Reminders are delivered by the OS as real notifications from a native trigger, even while the app is closed. Scheduled tasks (cron) resume a conversation and run a prompt: agent work runs while the app is active, and when inactive Kavi schedules a tap-to-wake notification instead of starting agent work headlessly. | Reminder store, recurrence/DST, and native trigger tests; scheduler engine, notification service, background maintenance, wake-race, and navigation-route tests. |
...
| Python workspace code execution                                                         | Yes | Yes     | Setup required    | Runs in a worker-backed Pyodide runtime. Network access is disabled per invocation unless explicitly enabled; supported HTTP is native-policy routed. Initial runtime and package downloads require network access. | Python bootstrap, native HTTP bridge, workspace persistence, effect receipt, and exact Release chat tests. |
