...
| Scheduled workflows and reminders                                                       | Yes | Yes     | Permission-gated  | Agent work runs while the app is active. When inactive, Kavi schedules a notification and the user taps it to run the due task; headless maintenance never starts arbitrary agent work.                             | Scheduler engine, notification service, background maintenance, wake-race, and navigation-route tests.     |
